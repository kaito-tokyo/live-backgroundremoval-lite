...
import AppKit                          // For NSImage / CGImage (macOS)
	import CoreVideo import Vision // For VNImageRequestHandler
	import XCTest

	final class SelfieSegmenterLandscapeWrapperTests:XCTestCase{

		// Expected model input/output dimensions
		let modelWidth = 256 let modelHeight = 144

		// MARK: - Test Error

		// Local error enum for test helpers
		enum TestError:Error{case fileNotFound(String) case maskLoadFailed(String) case maskDataExtractionFailed }
   ...
   */
		func testSegmentationSuccess_ValidImage_ReturnsCorrectMask() throws{
			...
			let segmenter = try SelfieSegmenterLandscapeWrapper()
				...
				guard let nsImage = Bundle(for :type(of:self)).image(forResource: "selfie001"), let cgImage = nsImage.cgImage(forProposedRect:nil, context:nil, hints:nil) else {XCTFail("Failed to load input image 'selfie001'.") throw TestError.fileNotFound("selfie001") } let inputHandler = VNImageRequestHandler(cgImage:cgImage)
																																									 ...
																																									 XCTAssertNotNil(resultArray, "perform(with:) returned nil.")
																																											 ...
																																											 guard let nsMask = Bundle(for :type(of:self)).image(forResource: "selfie001_mask"), let maskCGImage = nsMask.cgImage(forProposedRect:nil, context:nil, hints:nil) else {XCTFail("Failed to load mask image 'selfie001_mask'.") throw TestError.maskLoadFailed("selfie001_mask") }

																																																																 let pixelCount = modelWidth * modelHeight

																																																																 // 5. Validate and extract ground truth mask data
																																																																 XCTAssertEqual(maskCGImage.width, modelWidth, "Ground truth mask width is incorrect.") XCTAssertEqual(maskCGImage.height, modelHeight, "Ground truth mask height is incorrect.")

																																																																													       guard let truthMaskData = extractGrayscaleData(from:maskCGImage, width:modelWidth, height:modelHeight) else {XCTFail("Failed to extract pixel data from ground truth mask.") throw TestError.maskDataExtractionFailed }

																																																																																											    // 6. Convert MLMultiArray to [UInt8] array
																																																																																											    XCTAssertEqual(resultArray.dataType, .float32, "MLMultiArray must be Float32") var modelMaskData =[UInt8](repeating: 0, count:pixelCount) let floatPtr = UnsafeMutableBufferPointer <Float>(start:resultArray.dataPointer.assumingMemoryBound(to:Float.self), count:pixelCount) for i in 0.. <pixelCount {
  ...
  func testSegmentationSuccess_BGRAData_ReturnsCorrectMask() throws {
    ...
    let floatPtr = UnsafeMutableBufferPointer<Float>(start:resultArray.dataPointer.assumingMemoryBound(to:Float.self), count:pixelCount) for i in 0.. <pixelCount {
      let floatValue = floatPtr[i]
      let scaledValue = floatValue * 255.0
      modelMaskData[i] = UInt8(min(max(scaledValue, 0.0), 255.0))
    }
