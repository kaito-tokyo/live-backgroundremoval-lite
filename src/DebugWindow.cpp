...
void DebugWindow::videoRender()
{
	if (auto mainPluginContext = weakMainPluginContext.lock()) {
		...
		if (!readerMaskRoiR8 || readerMaskRoiR8->width != renderingContext->maskRoiWidth || readerMaskRoiR8->height != renderingContext->maskRoiHeight) {
		...
		if (!readerSubR8 || readerSubR8->width != renderingContext->gfWidthSub || readerSubR8->height != renderingContext->gfHeightSub) {
			readerSubR8 = std::make_unique<AsyncTextureReader>(renderingContext->gfWidthSub,
									  renderingContext->gfHeightSub, GS_R8);
		}

		if (!readerR16fSub || readerR16fSub->width != renderingContext->gfWidthSub || readerR16fSub->height != renderingContext->gfHeightSub) {
...
void DebugWindow::updatePreview()
{
	...
	const std::vector<std::string> bgrxTextures = {
		textureBgrxOriginalImage
	};
	const std::vector<std::string> r8Textures = {
		textureR8OriginalGrayscale,
		textureR8GFResult
	};
	const std::vector<std::string> bgrx256Textures = {
		textureBgrxSegmenterInput
	};
	const std::vector<std::string> r8MaskRoiTextures = {
		textureR8SegmentationMask
	};
	const std::vector<std::string> subR8Textures = {
		textureR8GFGuideSub,
		textureR8GFSourceSub
	};
	...
	if (std::find(bgrxTextures.begin(), bgrxTextures.end(), currentTexture) != bgrxTextures.end()) {
		image = QImage(readerBgrx->getBuffer().data(), readerBgrx->width, readerBgrx->height, QImage::Format_RGB32);
	} else if (std::find(r8Textures.begin(), r8Textures.end(), currentTexture) != r8Textures.end()) {
		image = QImage(readerR8->getBuffer().data(), readerR8->width, readerR8->height, QImage::Format_Grayscale8);
	} else if (std::find(bgrx256Textures.begin(), bgrx256Textures.end(), currentTexture) != bgrx256Textures.end()) {
		image = QImage(reader256Bgrx->getBuffer().data(), reader256Bgrx->width, reader256Bgrx->height, QImage::Format_RGB32);
	} else if (std::find(r8MaskRoiTextures.begin(), r8MaskRoiTextures.end(), currentTexture) != r8MaskRoiTextures.end()) {
		image = QImage(readerMaskRoiR8->getBuffer().data(), readerMaskRoiR8->width, readerMaskRoiR8->height, QImage::Format_Grayscale8);
	} else if (std::find(subR8Textures.begin(), subR8Textures.end(), currentTexture) != subR8Textures.end()) {
		image = QImage(readerSubR8->getBuffer().data(), readerSubR8->width, readerSubR8->height, QImage::Format_Grayscale8);
		...
		image = QImage(bufferSubR8.data(), readerR16fSub->width, readerR16fSub->height, QImage::Format_Grayscale8);
	}
