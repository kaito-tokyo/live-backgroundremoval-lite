...
void DebugWindow::videoRender()
{
	if (auto mainPluginContext = weakMainPluginContext.lock()) {
		...
		if (!readerBgrx || readerBgrx->width != renderingContext->width ||
			...
			readerBgrx = std::make_unique<BridgeUtils::AsyncTextureReader>(renderingContext->width,
									  renderingContext->height, GS_BGRX);
		}

		if (!readerR8 || readerR8->width != renderingContext->width ||
		    readerR8->height != renderingContext->height) {
			readerR8 = std::make_unique<BridgeUtils::AsyncTextureReader>(renderingContext->width,
		...
		if (!readerR32f || readerR32f->width != renderingContext->width ||
			...
			readerR32f = std::make_unique<BridgeUtils::AsyncTextureReader>(renderingContext->width,
		...
		if (!readerMaskRoiR8 || readerMaskRoiR8->width != renderingContext->maskRoiWidth ||
			...
			readerMaskRoiR8 = std::make_unique<BridgeUtils::AsyncTextureReader>(renderingContext->maskRoiWidth,
									       renderingContext->maskRoiHeight, GS_R8);
		}

		if (!reader256Bgrx) {
			reader256Bgrx = std::make_unique<BridgeUtils::AsyncTextureReader>(256, 256, GS_BGRX);
		}

		if (!readerSubR8 || readerSubR8->width != renderingContext->widthSub ||
		    readerSubR8->height != renderingContext->heightSub) {
			readerSubR8 = std::make_unique<BridgeUtils::AsyncTextureReader>(renderingContext->widthSub,
		...
		if (!readerR32fSub || readerR32fSub->width != renderingContext->widthSub ||
			...
			readerR32fSub = std::make_unique<BridgeUtils::AsyncTextureReader>(renderingContext->widthSub,
