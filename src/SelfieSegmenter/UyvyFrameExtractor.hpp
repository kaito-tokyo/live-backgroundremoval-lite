...
inline DownsampledDimensions calculate_downsampled_dims(std::size_t src_width, std::size_t src_height, std::size_t max_dim = 256) {
    if (src_width == 0 || src_height == 0) return {0, 0};
...
namespace range {
    struct Limited {
        static inline float scale_y(uint8_t y_val) {
            return (static_cast<float>(y_val) - 16.0f) * (255.0f / 219.0f);
        }
        static inline float scale_c(uint8_t c_val) {
            return (static_cast<float>(c_val) - 128.0f);
        }
    };
    struct Full {
        static inline float scale_y(uint8_t y_val) {
    ...
    void operator()(ChannelType dstR, ChannelType dstG, ChannelType dstB, DataType srcdata,
        ...
        for (std::size_t dy_img = 0; dy_img < img_height; ++dy_img) {
            ...
            for (std::size_t dx_img = 0; dx_img < img_width; ++dx_img) {
                ...
                const float g_comp = TColorCoefficients::CB_TO_G * cb_centered + TColorCoefficients::CR_TO_G * cr_centered;
